...
class ResidualBlock(nn.Module):
    """
    ResNet의 핵심인 잔차 블록(Residual Block).
    입력을 건너뛰어 더해주는(Skip Connection) 구조로 깊은 신경망도 안정적으로 학습하게 해줌.
    """
...
class OmokResNet(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256, num_blocks: int = 4):
        ...
        n_input_channels = observation_space.shape[0]  # 2채널 (내 돌, 상대 돌)
        base_channels = 64

        # 1. 초기 합성곱 계층 (특징 추출 시작)
        ...
        # 2. Residual Blocks 쌓기 (판 전체의 맥락 파악)
        blocks = []
...
def train_curriculum():
    # 이제 OmokCNN 대신 OmokResNet을 사용하고, 블록 개수를 설정해 줍니다.
    policy_kwargs = dict(
        features_extractor_class=OmokResNet,
        features_extractor_kwargs=dict(features_dim=256, num_blocks=4),
        normalize_images=False,
    )

    TB_LOG_PATH = "./tb_omok_resnet/"

    print("🔥 [1단계] 랜덤 상대 특훈 시작 (ResNet 탑재)")
    ...
    model.save("omok_resnet_stage1")
    env1.close()

    print("\n🔥 [2단계] 규칙형 튜터 상대 실전 특훈 시작 (ResNet 탑재)")
    ...
    model.save("omok_resnet_ultimate")
    env2.close()
